...
def get_student_groups():
    ...
    try:
        student_groups = {
            ...
            "English Learners": {
                "column": "englishlearner",
                "in": ["Y", "Yes", "True", 1],
            },
            "Students with Disabilities": {
                "column": "studentswithdisabilities",
                "in": ["Y", "Yes", "True", 1],
            },
            "Socioeconomically Disadvantaged": {
                "column": "socioeconomicallydisadvantaged",
                "in": ["Y", "Yes", "True", 1],
            },
            "Hispanic or Latino": {
                "column": "ethnicityrace",
                "in": ["Hispanic", "Hispanic or Latino"],
            },
            "White": {"column": "ethnicityrace", "in": ["White"]},
            "Black or African American": {
                "column": "ethnicityrace",
                "in": ["Black", "African American", "Black or African American"],
            },
            ...
            "Two or More Races": {
                "column": "ethnicityrace",
                "in": ["Two or More Races", "Multiracial", "Multiple Races"],
            },
            "Not Stated": {
                "column": "ethnicityrace",
                "in": ["Not Stated", "Unknown", ""],
            },
...
def get_assessment_filters():
    ...
    try:
        ...
        requested_assessments = (
            [a.strip() for a in assessments_param.split(",") if a.strip()]
            if assessments_param
            else []
        )
        ...
        if requested_assessments:
            filtered_filters = {
                k: v
                for k, v in assessment_filters.items()
                if k in requested_assessments
            }
        else:
        ...
        all_quarters = set()
        supports_grades = any(
            f.get("supports_grades", False) for f in filtered_filters.values()
        )
        supports_student_groups = any(
            f.get("supports_student_groups", False) for f in filtered_filters.values()
        )
        supports_race = any(
            f.get("supports_race", False) for f in filtered_filters.values()
        )
    ...
    try:
        ...
        if chart_index_path:
            ...
            analyses = analyze_charts_from_index(
                chart_index_path, output_dir, max_charts=max_charts
            )
        elif chart_paths:
        ...
        return (
            jsonify({"success": True, "analyses": analyses, "count": len(analyses)}),
            200,
        )
    ...
    try:
        ...
        if isinstance(chart_filters, str):
            ...
            try:
                ...
                print(
                    f"[Backend] Warning: Failed to parse chart_filters, using empty dict"
                )
                chart_filters = {}
        ...
        if not isinstance(chart_filters, dict):
            print(
                f"[Backend] Warning: chart_filters is not a dict ({type(chart_filters).__name__}), using empty dict"
            )
            chart_filters = {}
        ...
        print(
            f"[Backend] Data sources configured: NWEA={has_nwea}, iReady={has_iready}, STAR={has_star}"
        )
        ...
        try:
            ...
            if os.path.exists(temp_charts_dir):
                print(
                    f"[Backend] Cleaning up temp directory on error: {temp_charts_dir}"
                )
...
def create_slides():
    ...
    try:
        ...
        if deck_type:
            print(
                f"[Backend] Detected deck_type: {deck_type} from quarters: {quarters}"
            )
        else:
            print(
                f"[Backend] No deck_type detected from quarters: {quarters}, will use base reference decks"
            )

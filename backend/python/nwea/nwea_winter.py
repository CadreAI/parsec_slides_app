"""
NWEA Winter chart generation module
...
# Chart generation functions are now defined locally (moved from nwea_charts.py to match STAR structure)
...
# ---------------------------------------------------------------------
# Winter-specific Section 0: Predicted vs Actual CAASPP (Winter)
# ---------------------------------------------------------------------

def _prep_section0_winter(df, subject):
    """Prepare data for Section 0: Predicted vs Actual CAASPP - Winter version"""
    d = df.copy()
...
# ---------------------------------------------------------------------
# Section 0.1: Fall vs Winter Performance Snapshot (Winter-specific)
# ---------------------------------------------------------------------

def _prep_nwea_fall_winter_snapshot(df: pd.DataFrame, subject_str: str):
    ...
    # Ensure all quintiles exist for Fall/Winter
    ...
    # 10. Insight metrics from Fall → Winter
    if len(time_order) >= 2:
...
def plot_nwea_dual_subject_fall_winter_dashboard(
    ...
    for i, (pct_df, score_df, metrics, title) in enumerate(zip(pct_dfs, score_dfs, metrics_list, titles)):
        ...
        ax2.set_title("Avg RIT (Fall vs Winter)", fontsize=11, fontweight="bold", pad=10)
    ...
    fig.suptitle(f"{main_label} • Fall to Winter Performance Snapshot", fontsize=24, fontweight="bold", y=1)
    ...
    out_name = f"{scope.replace(' ', '_')}_NWEA_section0_1_dual_subject_fall_winter_snapshot.png"
...
def _save_cgp_chart(fig, scope_label, output_dir, cfg, section_num=4, suffix="cgp_fall_to_winter_dualpanel", chart_data=None):
    charts_dir = Path(output_dir) / "charts"
    folder_name = (
    ...
    out_name = f"{safe_scope}_NWEA_section{section_num}_{suffix}.png"
    out_path = out_dir / out_name
    hf._save_and_render(fig, out_path)
...
def _run_section5_winter(nwea_base, cfg, output_dir, scopes):
        ...
        for level in metrics["proj_order"]:
            ...
            if val >= LABEL_MIN_PCT:
                bar_ax.text(rect.get_x() + rect.get_width() / 2.0, cumulative + val / 2.0, f"{val:.1f}%",
                           ha="center", va="center", fontsize=11, fontweight="bold", color="#434343")
            cumulative += val
...
def plot_nwea_blended_dashboard(df, course_str, current_grade, window_filter, cohort_year,
    ...
    ax1.set_title("Overall Trends", fontsize=18, fontweight="bold", pad=45)
    ...
    fig.suptitle(f"{district_label} • Grade {hf.format_grade_label(current_grade)} • {course_str_for_title} • {window_filter} Performance Levels by Year",
                fontsize=24, fontweight="bold", y=1)
...
def main(nwea_data=None):
    ...
    for scope_df, scope_label, folder in scopes:
        try:
            ...
            for subj in ["Reading", "Mathematics"]:
                if not should_generate_subject(subj, chart_filters):
                    continue
                proj, act, metrics, _ = _prep_section0_winter(scope_df, subj)
                if proj is None:
                    continue
                payload[subj] = {"proj_pct": proj, "act_pct": act, "metrics": metrics}
            if payload:
    ...
    def _run_scope_section3_winter(scope_df, scope_label, school_raw):
        scope_df = scope_df.copy()
        scope_df["year"] = pd.to_numeric(scope_df["year"], errors="coerce")
    ...
    for scope_df, scope_label, folder in scopes:
        _run_scope_section3_winter(scope_df.copy(), scope_label, None if folder == "_district" else scope_df["schoolname"].iloc[0] if "schoolname" in scope_df.columns else None)
    
    # Section 0.1: Fall vs Winter Performance Snapshot (Winter-specific)
    print("\n[Section 0.1] Generating Fall vs Winter Performance Snapshot...")
    section0_1_paths = _run_section0_1_winter(nwea_base.copy(), cfg, args.output_dir)
    chart_paths.extend(section0_1_paths)
    
    # Section 4: Overall Growth Trends by Site (CGP + CGI) - Fall→Winter (Winter-specific)
    print("\n[Section 4] Generating Overall Growth Trends by Site (Fall→Winter CGP/CGI)...")
    section4_paths = _run_section4_winter(nwea_base.copy(), cfg, args.output_dir, scopes)
    chart_paths.extend(section4_paths)
    
    # Section 5: CGP/CGI Growth: Grade Trend + Backward Cohort - Fall→Winter (Winter-specific)
    print("\n[Section 5] Generating CGP/CGI Growth: Grade Trend + Backward Cohort (Fall→Winter)...")
    section5_paths = _run_section5_winter(nwea_base.copy(), cfg, args.output_dir, scopes)
    ...
    print(f"\n✅ Generated {len(unique_chart_paths)} unique NWEA Winter charts")
...
def generate_nwea_winter_charts(
    ...
    """
    Generate NWEA Winter charts (wrapper function for Flask backend)

...
def generate_iready_eoy_charts(
    ...
    try:
        ...
        if groups is not None:
            if isinstance(groups, list):
                if len(groups) == 0:
                    env["IREADY_EOY_STUDENT_GROUPS"] = "NONE"
                else:
                    env["IREADY_EOY_STUDENT_GROUPS"] = ",".join(str(g).strip() for g in groups if str(g).strip())
            elif isinstance(groups, str) and groups.strip():
                env["IREADY_EOY_STUDENT_GROUPS"] = groups.strip()
        
        # Handle races separately
        if races is not None:
            if isinstance(races, list):
                if len(races) == 0:
                    env["IREADY_EOY_RACE"] = "NONE"
                else:
                    env["IREADY_EOY_RACE"] = ",".join(str(r).strip() for r in races if str(r).strip())
            elif isinstance(races, str) and races.strip():
                env["IREADY_EOY_RACE"] = races.strip()
    except Exception:
        pass
    ...
    try:
        ...
        env["IREADY_EOY_COMPARE_WINDOWS"] = "Fall,Winter,Spring"
    ...
    try:
        ...
        if bool(cf.get("district_only")) is True:
            ...
            env["IREADY_EOY_SCOPE_MODE"] = "schools_only"
        ...
        if isinstance(schools, list) and len(schools) > 0:
            ...
            if "IREADY_EOY_SCOPE_MODE" not in env:

            ...
            if "section_title" not in payload:
                ...
                if sec is not None:
                    ...
                    IREADY_TITLES = {
                        ...
                        "4": "Spring i-Ready Mid/Above → % CAASPP Met/Exceeded",
...
def plot_iready_subject_dashboard_by_group(
    ...
    for i, subj in enumerate(subjects):
        ...
        ax2.set_title(
            "Average Scale Score", fontsize=14, fontweight="bold", pad=15, color="#111111"
        )
...
# The runner passes selected grades as: IREADY_EOY_GRADES="3,4,5"
...
def _plot_mid_above_to_cers_faceted(scope_df, scope_label, folder_name, preview=False):
    ...
    fig.suptitle(
        f"{scope_label} \n Spring i-Ready Mid/Above → % CAASPP Met/Exceeded",
        fontsize=20,
        fontweight="bold",
        y=1.02,
    )
...
def _plot_fall_spring_grouped_stacked(
    ...
    _write_chart_data(
        out_path,
        {
            "chart_type": f"iready_eoy_section{section_num}_window_compare",
            "section": section_num,
            "scope": scope_label,
...
def _plot_grouped_typ_stretch(
    ...
    if section_num is not None:
        _write_chart_data(
            out_path,
            {
                "chart_type": f"iready_eoy_section{section_num}_median_progress",
                "section": section_num,

...
# Development mode flag (set to True for interactive preview, False for batch processing)
DEV_MODE = False
...
def save_chart(
    ...
    *,
    ...
    for ext in fmt:
        ...
        # Final verification
        if not fpath.exists():
            raise IOError(f"Chart file was not created: {fpath}")
        if fpath.stat().st_size == 0:
            raise IOError(f"Chart file is empty: {fpath}")

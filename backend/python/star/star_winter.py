...
def plot_section_1_1_dual_subject(df, scope_label, folder, output_dir, school_raw=None, preview=False):
    """Plot Section 1.1: Fall → Winter Performance Progression - Dual Subject"""
    ...
    chart_data = {
        "chart_type": "star_winter_section1_1_fall_winter_progression_dual_subject",
        "section": 1.1,
        "scope": scope_label,
...
def plot_section_1_2_for_grade_dual_subject(df, scope_label, folder, output_dir, grade, school_raw=None, preview=False):
    """Plot Section 1.2 for a single grade - Dual Subject Fall → Winter Progression"""
    ...
    chart_data = {
        "chart_type": "star_winter_section1_2_fall_winter_progression_by_grade_dual_subject",
        "section": 1.2,
        "scope": scope_label,
        "window_filter": "Fall/Winter",
...
def plot_section_1_3_for_group_dual_subject(df, scope_label, folder, output_dir, group_name, group_def, school_raw=None, preview=False):
    """Plot Section 1.3 for a single student group - Dual Subject Fall → Winter Progression"""
    ...
    chart_data = {
        "chart_type": "star_winter_section1_3_fall_winter_progression_by_group_dual_subject",
        "section": 1.3,
        "scope": scope_label,
        "window_filter": "Fall/Winter",
...
def _prep_star_sgp_trend_district_overview(df, subject_str):
    ...
    # Use fallback mechanism: prefer FALL_WINTER, fallback to FALL_SPRING
    vector_used = _get_best_sgp_vector(d_with_sgp, preferred="FALL_WINTER", fallback="FALL_SPRING")
    ...
    if vector_used == "FALL_WINTER":
        ...
        print(f"[Section 4 Overview] No FALL_WINTER data, using FALL_SPRING SGP data for {subject_str}")
    else:
...
def plot_district_sgp_overview_single_subject_winter(
    ...
    """
    # Determine subject title - properly handle Spanish Reading
    ...
    out_dir.mkdir(parents=True, exist_ok=True)
    safe_scope = scope_label.replace(" ", "_")
    # Ensure Spanish Reading gets a distinct filename
    if 'spanish' in subject_str.lower():
        safe_subj = 'reading_spanish'
    elif subject_str.lower() in ['math', 'mathematics']:
        safe_subj = 'math'
    else:
        safe_subj = 'reading'
...
def plot_district_sgp_overview_dual_subject_winter(
    df, scope_label, folder, output_dir, window_filter="Winter", preview=False
):
    """
    ...
    # Track chart with data
    chart_data = {
        "chart_type": "star_winter_section4_sgp_overview_dual_subject",
        "section": 4,
        "scope": scope_label,
        "window_filter": window_filter,
        "sgp_data": {
...
def _prep_star_sgp_data_winter(df, subject_str, current_grade, window_filter):
    ...
    # Check for SGP columns - try FALL_WINTER first, fallback to FALL_SPRING
    if "current_sgp_vector" in d.columns and "current_sgp" in d.columns:
        sgp_vector_used = _get_best_sgp_vector(d, preferred="FALL_WINTER", fallback="FALL_SPRING")
        if sgp_vector_used:
            d = d[d["current_sgp_vector"] == sgp_vector_used].copy()
        else:
            # No SGP data available
            pass
...
def plot_star_sgp_growth_winter(
    ...
    if not sgp_df_grade.empty:
        ...
        ax1.set_xticks(x)
        # Determine SGP label based on available data
        sgp_label = "Median Fall→Winter SGP"
        ax1.set_ylabel(sgp_label, fontsize=11, fontweight="bold")
    ...
    if not cohort_df.empty:
        ...
        # Determine SGP label based on available data
        sgp_label = "Median Fall→Winter SGP"
        ax2.set_ylabel(sgp_label, fontsize=11, fontweight="bold")
    ...
    # Main title - determine SGP vector label dynamically
    # Note: For Section 5, we use FALL_WINTER with FALL_SPRING fallback
    # The actual vector used is determined in _prep_star_sgp_data_winter
    sgp_title_label = "Fall→Winter SGP"
    fig.suptitle(
        f"{scope_label} • {subject_str} • Grade {current_grade} • {sgp_title_label}",
        fontsize=20,
        fontweight="bold",
        y=1.02,
    )
...
def _get_best_sgp_vector(df: pd.DataFrame, preferred: str = "FALL_WINTER", fallback: str = "FALL_SPRING") -> str | None:
    """
    ...
    Args:
        ...
        preferred: Preferred SGP vector (default: "FALL_WINTER")
...
def _prep_star_sgp_latest_by(
    ...
    # SGP window - use fallback if sgp_vector is FALL_WINTER
    if "current_sgp_vector" not in d.columns:
        return pd.DataFrame()
    
    if sgp_vector == "FALL_WINTER":
        # Use fallback mechanism
        fallback_vector = "FALL_SPRING"
...
def plot_section_9_sgp_by_school_winter(
    ...
    # Determine window label based on actual vector used
    window_label = "Fall→Winter" if sgp_vector == "FALL_WINTER" else "Fall→Spring"
...
def main(star_data=None):
        ...
        try:
            ...
            for subj in ["Reading", "Mathematics"]:
                if not should_generate_subject(subj, chart_filters):
                    print(f"[Section 0] Skipping {subj} for {scope_label} (filtered out)")
                    continue
                proj, act, metrics, year = _prep_section0_star_winter(scope_df, subj)
                if proj is None:
                    print(f"[Section 0] No data for {subj} in {scope_label} - skipping this subject")
                    continue
                payload[subj] = {"proj_pct": proj, "act_pct": act, "metrics": metrics}
            if payload:
                chart_path = _plot_section0_star_winter(scope_label, folder, payload, args.output_dir, preview=hf.DEV_MODE)
                if chart_path:
                    chart_paths.append(chart_path)
            else:
                print(f"[Section 0] No data available for {scope_label} - skipping chart")
        except Exception as e:
    ...
    # Section 1.1: Fall → Winter Performance Progression (dual-subject dashboard)
    ...
    for scope_df, scope_label, folder in scopes:
        if folder == "_district":
            ...
            try:
                chart_path = plot_district_sgp_overview_dual_subject_winter(
                    scope_df.copy(),
                    scope_label,
                    folder,
                    args.output_dir,

...
export function sqlCers(tableId: string, _excludeCols?: string[], _filters?: { districts?: string[]; years?: number[]; schools?: string[] }): string {
    return `
    SELECT DISTINCT *
    FROM \`${tableId}\`
...
export function sqlIab(tableId: string, _excludeCols?: string[], _filters?: { districts?: string[]; years?: number[]; schools?: string[] }): string {
    return `
    SELECT DISTINCT *
    FROM \`${tableId}\`
    WHERE LocalID <> 99999
...
export function sqlCalpads(tableId: string, _excludeCols?: string[], _filters?: { districts?: string[]; years?: number[]; schools?: string[] }): string {
    return `
    SELECT DISTINCT *
    FROM \`${tableId}\`
    WHERE Year >= (
...
export function sqlStar(tableId: string, excludeCols: string[] = [], _filters?: { districts?: string[]; years?: number[]; schools?: string[] }): string {
    const dynamicExcludes = excludeCols.length > 0 ? ',\n        ' + excludeCols.join(',\n        ') : ''
...
export function sqlIready(tableId: string, excludeCols: string[] = [], _filters?: { districts?: string[]; years?: number[]; schools?: string[] }): string {
    const dynamicExcludes = excludeCols.length > 0 ? ',\n        ' + excludeCols.join(',\n        ') : ''
